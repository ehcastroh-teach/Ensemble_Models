...
A two-notebook series on exploratory data analysis, feature engineering, and ensemble learning. The notebooks use the Titanic passenger dataset as a running case study, progressing from raw data inspection through preprocessing to a competitive ensemble of AutoML predictions. Homework notebooks extend each lesson to new datasets so you can practice the same techniques independently.
...
| `assets/` | Expected output images used by the homework notebooks for self-grading |
| `images/` | Banner and thumbnail images used by this README |
...
- Titles are extracted from passenger names using a regex (`[A-Za-z]+\.`) because title encodes social status and age category simultaneously - two factors that directly influenced lifeboat priority. Rare titles are grouped into a single "Rare" category to avoid overfitting sparse classes.
...
- Fare is binned into quartiles because the raw distribution is right-skewed with extreme outliers. The quartile rank is robust to the skew and still preserves the ordering signal.
- Cabin is dropped (77% missing) and Ticket is dropped (no extractable structure), reducing noise rather than risking spurious learned patterns.

**Notebook 2** explains ensemble theory first, then demonstrates it. The key ideas are:

- Ensemble methods reduce error when constituent models make *different* mistakes - model diversity is what makes averaging better than any individual model. This is why combining three separate AutoML frameworks (each with its own search strategy) outperforms running any one of them alone.
- LightAutoML and FlaML both search over multiple model families within a time budget, but use different search strategies. LightAutoML builds an internal blend; FlaML returns the single best configuration found within the budget.
- Simple averaging of probability outputs from three AutoML systems (LightAutoML, FlaML, and H2O.ai) achieves 78.2% Kaggle accuracy, beating each system run alone.
- Threshold tuning shifts the decision boundary away from 0.5 to adjust the precision/recall balance. The notebook demonstrates why the threshold must be tuned on a validation set and never on the held-out test set, since tuning on the test set would leak label information.
...
**Colab:** Both notebooks run on Google Colab with no local setup. Upload `train.csv`, `test.csv`, and `h2oai_experiment_macewube_test_predictions.csv` to your Colab session before executing any cells.
...
- Ensemble theory foundations: *The Elements of Statistical Learning* by Hastie, Tibshirani, and Friedman
